...
extension SCNetworkReachabilityFlags {
    ...
    var description: String {
        let WDescription = isOnWWANFlagSet ? "W" : "-"
        let RDescription = isReachableFlagSet ? "R" : "-"
        let cDescription = isConnectionRequiredFlagSet ? "c" : "-"
        let tDescription = isTransientConnectionFlagSet ? "t" : "-"
        let iDescription = isInterventionRequiredFlagSet ? "i" : "-"
        let CDescription = isConnectionOnTrafficFlagSet ? "C" : "-"
        let DDescription = isConnectionOnDemandFlagSet ? "D" : "-"
        let lDescription = isLocalAddressFlagSet ? "l" : "-"
        let dDescription = isDirectFlagSet ? "d" : "-"
        
        return  "\(WDescription)\(RDescription) " +
                "\(cDescription)\(tDescription)" +
                "\(iDescription)\(CDescription)\(DDescription)" +
                "\(lDescription)\(dDescription)"
    }
}
